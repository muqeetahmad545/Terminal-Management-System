...
const MerchantConfigurations = () => {
	...
	//? HOOKS
	const { configUseCases, paramId } = useMerchant();
	...
	return (
		<div className='p-5 bg-white rounded-md'>
			<div className='flex justify-between items-center'>
				<AppListHeader
					appListTitle={`${
						state.merchant_id + '_' + state.merchant?.name || 'Merchant'
					} Configurations`}
				...
				{activeIndex == 1 && (
					<div className='grid grid-cols-2 gap-5 text-sm'>
						...
						<AppCardSettings
							...
							appCardIsActive={state['qr_code_receipts']}
							appCardonClick={() => handleCardClick('qr_code_receipts')}
						/>
					</div>
				)}
				{/* PAYMENT SETTINGS */}
